...
pub fn start_connection(app: &mut App) {
    ...
    let params = StartConnectionParams {
        ...
        llama_url: app.llama_url.clone(),
        llama_temperature: app.llama_temperature,
    };
    let conn_slot: Rc<std::cell::RefCell<Option<ConnectionSlot>>> =
        Rc::new(std::cell::RefCell::new(None));
    let conn_slot_writer = Rc::clone(&conn_slot);

    let (rt_tx, rt_rx) = mpsc::unbounded_channel::<llama_lifecycle::LlamaRuntimeCommand>();
    if matches!(app.provider, crate::Provider::Llamacpp) {
        app.llama_runtime_tx = Some(rt_tx.clone());
    }
    ...
    tokio::task::spawn_local(async move {
        match params.provider {
            ...
            crate::Provider::Llamacpp => {
                llama_lifecycle::run_llama_task(params, conn_slot_writer, rt_tx, rt_rx).await;
            }
        }
    });
