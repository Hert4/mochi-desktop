...
impl PasteBurstDetector {
    ...
    pub fn on_char(&mut self, ch: char, now: Instant) -> CharAction {
        // Non-ASCII characters (Vietnamese diacritics, CJK, emoji, etc.) come
        // from IME composition, not paste. Reset burst state so subsequent
        // ASCII typing isn't poisoned by IME's rapid emission, and pass the
        // char through directly.
        if !ch.is_ascii() {
            if matches!(self.state, BurstState::Buffering) {
                self.buffer.push(ch);
                self.last_char_time = Some(now);
                return CharAction::Consumed;
            }
...
mod tests {
    ...
    #[test]
    fn non_ascii_chars_passthrough_even_when_fast() {
        let mut d = PasteBurstDetector::new();
        let t0 = Instant::now();
        // Vietnamese diacritic arriving fast after another non-ASCII char must
        // not get swallowed by the burst detector. IME composition output is
        // not paste.
        assert_eq!(d.on_char('đ', t0), CharAction::Passthrough('đ'));
        let t1 = fast(t0, 1);
        assert_eq!(d.on_char('ư', t1), CharAction::Passthrough('ư'));
        let t2 = fast(t1, 1);
        assert_eq!(d.on_char('ợ', t2), CharAction::Passthrough('ợ'));
        let t3 = fast(t2, 1);
        assert_eq!(d.on_char('c', t3), CharAction::Passthrough('c'));
    }
