...
// Mochi-flavored tips shown on the welcome screen.
const WELCOME_TIPS: &[&str] = &[
    "Type /help to see slash commands available in this build",
    "Switch the LLM backend with --provider llamacpp for a fully local session",
    "Drop a SKILL.md under ~/.mochi/skills/<name>/ to give Mochi a persona",
    "Use /memory remember concept SLUG CONTENT so Mochi remembers facts across sessions",
    "Behavioral memories are scoped to the active skill — Mochi acts differently per persona",
    "Pick a different pet character with --pet bunny|frog|robot|dragon (more coming)",
    "Run `mochi chat` for a no-TUI REPL that streams from llama.cpp directly",
    "/memory profile <text> sets your one-paragraph user bio Mochi reads every session",
    "Start llama-server on a non-8080 port (Mochi defaults to 8765)",
    "Keep responses honest: ask Mochi for tradeoffs and risks, not just an answer",
];
...
mod tests {
    ...
    fn assistant_split_paragraph_inserts_a_structural_blank_line_between_blocks() {
        ...
        assert_eq!(rendered.first().map(String::as_str), Some("Mochi"));
    ...
    fn assistant_message_can_render_without_trailing_separator() {
        ...
        assert_eq!(render_lines_to_strings(&lines), vec!["Mochi".to_owned(), "hello".to_owned()]);
